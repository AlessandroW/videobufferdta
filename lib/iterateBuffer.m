function [pst Uneg Upos err] = iterateBuffer(T, segd, mb, sb, mv, sv, covbv, p, q, init)
...
if T<Inf
    ...
    B.x = linspace(logninv(0.00001,mu2,sigma2),logninv(0.99999,mu2,sigma2),1e4);
    B.prob = lognpdf(B.x,mu2,sigma2);
    B.prob = B.prob/sum(B.prob);
%    mtarget = mb*segd*((1./B.x)*B.prob');
    starget = sqrt(((mb*segd)^2+(sb*segd)^2)*((1./(B.x.^2))*B.prob')-(mb*segd)^2*(((1./B.x)*B.prob'))^2);
    mtarget = mb*segd/mv - (segd*covbv)/mv/mv + (mb*segd)*sv*sv/mv/mv/mv;
%    starget = sqrt(((sb*segd)^2)/mv/mv - 2*(mb*segd)*(segd*covbv)/mv/mv/mv + ((mb*segd)^2)*sv*sv/mv/mv/mv/mv);
...
%     end
     ...
     m = mtarget;
     s = starget;   
     while isnan(err) || sqrt(err^2)/(mtarget) > 0.001
              vd = starget*starget;
              md = m;
              mud = log((md^2)/sqrt(vd+md^2));
              sigmad = sqrt(log(vd/(md^2)+1));
              d.x = linspace(0,360,3e4);
              d.prob = lognpdf(d.x,mud,sigmad);
              d.prob = d.prob/sum(d.prob);
              err = d.x*d.prob'-mtarget;
              m = m + err;
     end
